...
export default class DerivativeSvc extends BaseSvc {
  ...
  postJob (token, oAuth, payload) {

    var postJob = new Forge.JobPayload()
    postJob.input = new Forge.JobPayloadInput()
    postJob.input.urn = payload.objectKey
    postJob.xAdsForce = true;
    postJob.output = new Forge.JobPayloadOutput(
        [new Forge.JobSvfOutputPayload()]
    );
    postJob.output.formats[0].type = 'svf';
    postJob.output.formats[0].views = ['2d', '3d'];


    return this._derivativesAPI.translate (postJob,{}, oAuth, token)
  }
