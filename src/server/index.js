...
import SensorDataAPI from './api/endpoints/sensors'
...
//HL Services
import Neo4jSvc from './api/services/Neo4jSvc'
import SensorDataProxySvc from './api/services/SensorDataProxySvc'
...
if (process.env.NODE_ENV === 'development') {
  ...
  app.use(session({
    secret: 'AFFCA0C6-6E12-479E-9B84-957D62539B83',
    cookie: {
      secure: false,
...
const neo4jSvc = new Neo4jSvc(config.database)
neo4jSvc.connect()
const sensorDataProxySvc = new SensorDataProxySvc()
...
ServiceManager.registerService(neo4jSvc)
ServiceManager.registerService(sensorDataProxySvc)
///////////////////////////////////////////////////////////
...
function runServer(app) {
  ...
  try {
   ...
   try {
      if (process.env.NODE_ENV === 'development') {
        key = fs.readFileSync('cert/devlocal.key')
        cert = fs.readFileSync( 'cert/devlocal.cert' )
      }
      else{
        key = fs.readFileSync('/cert/hlmepdc.key')
        cert = fs.readFileSync( '/cert/hlmepdc.cert' )
      }
    } catch (e) {
    ...
    const server = app.listen(process.env.PORT || config.server_port || 3000, () => {

      const httpsSvr = https.createServer(options, app).listen(3001);
